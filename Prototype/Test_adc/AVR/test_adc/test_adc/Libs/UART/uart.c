...
void usart0_init(void) {
	USART_DDR &= ~(1<<USART0_RX_PIN);
	USART_PORT |= (1<<USART0_RX_PIN);
...
void usart1_init(void) {
	USART_DDR &= ~(1<<USART1_RX_PIN);
	USART_PORT |= (1<<USART1_RX_PIN);
	rbuffer_init(&p_UART1_meta->rb_tx);                             // Init Rx buffer
	rbuffer_init(&p_UART1_meta->rb_rx);                             // Init Tx buffer
	UBRR1H=0;
	UBRR1L=51;
	UCSR1B |= (1<<RXEN1)|(1<<TXEN1)|(1<<RXCIE1);					//enable TX, RX, RX interrupt
...
void usart1_close() {
	...
	while(!(UCSR1A & (1 << TXC1)));										// Wait for Tx unit to transmit the LAST character of ringbuffer

	_delay_ms(200);                                             // Extra safety for Tx to finish!

	UCSR1B &= ~( (1<<RXEN1)|(1<<TXEN1)|(1<<RXCIE1) | (1<<	UDRIE1));					//disable TX, RX, RX interrupt
	UCSR1C &= (1<<UCSZ11) | (1<<UCSZ01);
}
